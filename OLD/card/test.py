...
def grasp_hold_actions(env):
    ...
    grasp = env._grasp_joint_pos   # ← the per-joint tensor built in __init__, not the raw cfg dict
...
while simulation_app.is_running():
    # actions = torch.zeros(env.num_envs, 16, device=env.device)   # hold the grasp pose
...
# print("RESET")
# print("=" * 60)
# print(f"obs['policy'] shape: {obs['policy'].shape}")   # (num_envs, 42)
...
# #     print(f"  terminated.sum():  {terminated.sum().item()}  (envs that dropped the credit card)")
...
# obs — a dict, not a bare tensor. Policy network reads obs["policy"], shape (num_envs, 42).
# rew — tensor, shape (num_envs,), one scalar reward per environment.
# terminated — bool tensor, shape (num_envs,). True where the credit card dropped past credit_card_drop_height_thresh.

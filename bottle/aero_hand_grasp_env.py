...
from bottle_cfg import BOTTLE_CFG
...
class AeroHandGraspEnvCfg(DirectRLEnvCfg):
    ...
    observation_space = 68          # 16 pos + 16 vel + 16 torque + 7 prev-action + 3 bottle pos
                                     # + 3 bottle linvel + 3 bottle angvel + 4 bottle quat (see _get_observations)
    state_space = 0
    ...
    bottle_cfg: RigidObjectCfg = BOTTLE_CFG.replace(prim_path="/World/envs/env_.*/Bottle")
    ...
    grasp_joint_pos = {
        "right_thumb_cmc_abd":  1.5, # base of thumb
        ...
        "right_index_mcp_flex": 1.1,
        "right_index_pip":      0.6,
        "right_index_dip":      0.6,

        "right_middle_mcp_flex": 1.1,
        "right_middle_pip":      0.6,
        "right_middle_dip":      0.6,

        "right_ring_mcp_flex": 1.1,
        "right_ring_pip":      0.6,
        "right_ring_dip":      0.6,

        "right_pinky_mcp_flex": 1.1,
        "right_pinky_pip":      0.5,
        "right_pinky_dip":      0.6,
    }

    bottle_drop_height_thresh = 0.15    # meters of downward drift counted as "dropped"
    ...
    rew_scale_bottle_height = 5.0      # reward each step the bottle is still considered "held"
    rew_scale_bottle_still = -0.1      # penalize bottle linear speed -> encourage a stable, non-wobbly grasp
...
class AeroHandGraspEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: AeroHandGraspEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # per-env bottle height captured at reset time, used to detect "has it dropped"
        self._initial_bottle_height = torch.zeros(self.num_envs, device=self.device)

    def _setup_scene(self):
        self.hand = Articulation(self.cfg.hand_cfg)
        self.bottle = RigidObject(self.cfg.bottle_cfg)
    ...
    def _get_observations(self) -> dict:
        ...
        # Bottle pose relative to the hand's articulation root. NOTE: this uses the ROOT,
        ...
        bottle_pos_rel = self.bottle.data.root_pos_w - hand_root_pos
        bottle_lin_vel = self.bottle.data.root_lin_vel_w
        bottle_ang_vel = self.bottle.data.root_ang_vel_w
    ...
    def _get_rewards(self) -> torch.Tensor:
        bottle_height = self.bottle.data.root_pos_w[:, 2] - self.scene.env_origins[:, 2]
        bottle_speed = torch.norm(self.bottle.data.root_lin_vel_w, dim=-1)

        dropped = (self._initial_bottle_height - bottle_height) > self.cfg.bottle_drop_height_thresh
        held = ~dropped
        ...
        rew_held = self.cfg.rew_scale_bottle_height * held.float()
        rew_still = self.cfg.rew_scale_bottle_still * bottle_speed
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # --- Hand: spawn already closed around the bottle (full 16-joint tuned pose) ---
        ...
        # --- Bottle: spawn positioned inside the grasp ---
        default_bottle_root = self.bottle.data.default_root_state[env_ids].clone()
        default_bottle_root[:, :3] += self.scene.env_origins[env_ids]
        self.bottle.write_root_pose_to_sim(default_bottle_root[:, :7], env_ids)
        self.bottle.write_root_velocity_to_sim(default_bottle_root[:, 7:], env_ids)

        self._initial_bottle_height[env_ids] = default_bottle_root[:, 2] - self.scene.env_origins[env_ids, 2]

...
#     observation_space = 68          # 16 pos + 16 vel + 16 torque + 7 prev-action + 3 mug pos
#                                      # + 3 mug linvel + 3 mug angvel + 4 mug quat (see _get_observations)
#     state_space = 0
...
#     mug_cfg: RigidObjectCfg = MUG_CFG.replace(prim_path="/World/envs/env_.*/Mug")
...
#     grasp_joint_pos = {
#         "right_thumb_cmc_abd":  1.4, # base of thumb
...
#     mug_drop_height_thresh = 0.15    # meters of downward drift counted as "dropped"
...
#     rew_scale_mug_height = 5.0      # reward each step the mug is still considered "held"
#     rew_scale_mug_still = -0.1      # penalize mug linear speed -> encourage a stable, non-wobbly grasp
...
#         # per-env mug height captured at reset time, used to detect "has it dropped"
#         self._initial_mug_height = torch.zeros(self.num_envs, device=self.device)
...
#         # Mug pose relative to the hand's articulation root. NOTE: this uses the ROOT,
...
#         mug_pos_rel = self.mug.data.root_pos_w - hand_root_pos
#         mug_lin_vel = self.mug.data.root_lin_vel_w
#         mug_ang_vel = self.mug.data.root_ang_vel_w
#         mug_quat = self.mug.data.root_quat_w
...
#             print("mug_pos_rel:", [f"{v:.3f}" for v in mug_pos_rel[env0].tolist()])
#             print("mug_lin_vel:", [f"{v:.3f}" for v in mug_lin_vel[env0].tolist()])
#             print("mug_ang_vel:", [f"{v:.3f}" for v in mug_ang_vel[env0].tolist()])
#             print("mug_quat:", [f"{v:.3f}" for v in mug_quat[env0].tolist()])
#             print("obs:", [f"{v:.3f}" for v in obs[env0].tolist()])
#             print("7 joint positions: ", [f"{v:.3f}" for v in self._grasp_actuation_pos.tolist()])
#             print("=" * 70)

#         return {"policy": obs}

#     def _get_rewards(self) -> torch.Tensor:
#         mug_height = self.mug.data.root_pos_w[:, 2] - self.scene.env_origins[:, 2]
#         mug_speed = torch.norm(self.mug.data.root_lin_vel_w, dim=-1)

#         dropped = (self._initial_mug_height - mug_height) > self.cfg.mug_drop_height_thresh
#         held = ~dropped
...
#         rew_held = self.cfg.rew_scale_mug_height * held.float()
#         rew_still = self.cfg.rew_scale_mug_still * mug_speed
...
#         # --- Hand: spawn already closed around the mug (full 16-joint tuned pose) ---
...
#         # --- Mug: spawn positioned inside the grasp ---
#         default_mug_root = self.mug.data.default_root_state[env_ids].clone()
#         default_mug_root[:, :3] += self.scene.env_origins[env_ids]
#         self.mug.write_root_pose_to_sim(default_mug_root[:, :7], env_ids)
#         self.mug.write_root_velocity_to_sim(default_mug_root[:, 7:], env_ids)

#         self._initial_mug_height[env_ids] = default_mug_root[:, 2] - self.scene.env_origins[env_ids, 2]

...
class AeroHandGraspEnvCfg(DirectRLEnvCfg):
    ...
    observation_space = 68          # 16 pos + 16 vel + 16 torque + 7 prev-action + 3 pen pos
                                     # + 3 pen linvel + 3 pen angvel + 4 pen quat (see _get_observations)
    state_space = 0
    ...
    pen_cfg: RigidObjectCfg = PEN_CFG.replace(prim_path="/World/envs/env_.*/Pen")
    ...
    grasp_joint_pos = {
        "right_thumb_cmc_abd":  0.5, # base of thumb
        "right_thumb_cmc_flex": 1.9, # moves thumb across the palm
        "right_thumb_mcp":      0.4, # moves thumb up/down
        "right_thumb_ip":       0.2, # tip of thumb

        "right_index_mcp_flex": 0.4,
        "right_index_pip":      1.6,
        "right_index_dip":      0.4,
    ...
    pen_drop_height_thresh = 0.15    # meters of downward drift counted as "dropped"
    ...
    rew_scale_pen_height = 5.0      # reward each step the pen is still considered "held"
    rew_scale_pen_still = -0.1      # penalize pen linear speed -> encourage a stable, non-wobbly grasp
...
class AeroHandGraspEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: AeroHandGraspEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # per-env pen height captured at reset time, used to detect "has it dropped"
        self._initial_pen_height = torch.zeros(self.num_envs, device=self.device)
    ...
    def _get_observations(self) -> dict:
        ...
        # Pen pose relative to the hand's articulation root. NOTE: this uses the ROOT,
        ...
        pen_pos_rel = self.pen.data.root_pos_w - hand_root_pos
        pen_lin_vel = self.pen.data.root_lin_vel_w
        pen_ang_vel = self.pen.data.root_ang_vel_w
        ...
        if obs.shape[0] > 0:
            ...
            print("pen_pos_rel:", [f"{v:.3f}" for v in pen_pos_rel[env0].tolist()])
            print("pen_lin_vel:", [f"{v:.3f}" for v in pen_lin_vel[env0].tolist()])
            print("pen_ang_vel:", [f"{v:.3f}" for v in pen_ang_vel[env0].tolist()])
            print("pen_quat:", [f"{v:.3f}" for v in pen_quat[env0].tolist()])
    ...
    def _get_rewards(self) -> torch.Tensor:
        pen_height = self.pen.data.root_pos_w[:, 2] - self.scene.env_origins[:, 2]
        pen_speed = torch.norm(self.pen.data.root_lin_vel_w, dim=-1)

        dropped = (self._initial_pen_height - pen_height) > self.cfg.pen_drop_height_thresh
        held = ~dropped
        ...
        rew_held = self.cfg.rew_scale_pen_height * held.float()
        rew_still = self.cfg.rew_scale_pen_still * pen_speed
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # --- Hand: spawn already closed around the pen (full 16-joint tuned pose) ---
        ...
        # --- Pen: spawn positioned inside the grasp ---
        default_pen_root = self.pen.data.default_root_state[env_ids].clone()
        default_pen_root[:, :3] += self.scene.env_origins[env_ids]
        self.pen.write_root_pose_to_sim(default_pen_root[:, :7], env_ids)
        self.pen.write_root_velocity_to_sim(default_pen_root[:, 7:], env_ids)

        self._initial_pen_height[env_ids] = default_pen_root[:, 2] - self.scene.env_origins[env_ids, 2]
